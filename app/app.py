...
def make_user_response(tasks, user_id):
    ...
    return {"tasks": tasks, "username": user_details[0]["username"]}
...
tasks = [
    {
        ...
        "description": "Click to edit. Put your description here..",
        "status": "To Do",
    }
]
...
def sample_to_list():
    ...
    if not check_cookies(request):
        ...
        if new_user_response["status"] is True:
            response = make_response(
                jsonify(make_user_response(new_user_response["tasks"], new_user_id)),
                200,
            )
        ...
        if not user_tasks:
            ...
            if not user:
                ...
                response = make_response(
                    jsonify(
                        make_user_response(new_user_response["tasks"], new_user_id)
                    ),
                    200,
                )
                response.headers["Set-Cookie"] = f"USER_ID={str(new_user_id)}; Path=/"
